    ...
    >
      ...
      <div class="card-content p-4 flex flex-col flex-grow">
        ...
        <div class="mt-auto flex justify-evenly items-center">
          ...
          <button @click="toggleFavorite(product.id)" class="flex items-center">
            <svg 
              xmlns="http://www.w3.org/2000/svg" 
              fill="currentColor" 
              :class="{'text-gray-300': !isFavorite(product.id), 'text-red-500': isFavorite(product.id)}" 
              class="w-6 h-6" 
              viewBox="0 0 24 24"
            >
          ...
          <button 
            ...
            class="bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:ring-2 focus:ring-gray-400 focus:ring-opacity-75 transition duration-200"
          >
            Add To Cart +
          </button>
        </div>
